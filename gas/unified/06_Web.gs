...
function doGet(event) {
  try {
    ...
    if (params.diagnostic === '1') return v21DiagnosticPage_();
    if (params.tool === 'hub') return HtmlService.createHtmlOutputFromFile('HubEditor').setTitle('HUB 圖檔編輯器').addMetaTag('viewport','width=device-width, initial-scale=1');
    ...
    return template.evaluate().setTitle('PSS AI-PMO V23.0.0 統整版').addMetaTag('viewport','width=device-width, initial-scale=1');
  } catch (error) {
...
function v21DiagnosticPage_() {
  ...
  try{HtmlService.createHtmlOutputFromFile('HubEditor').getContent();checks.push('HubEditor.html：存在');}catch(error){checks.push('HubEditor.html：'+error.message);}
  try{HtmlService.createHtmlOutputFromFile('TenantEditor').getContent();checks.push('TenantEditor.html：存在');}catch(error){checks.push('TenantEditor.html：'+error.message);}
...
function apiV21(request) {
  ...
  var routes = {
    ...
    restoreWork: function(){ return restoreWorkItemV23(payload); },
    workCalendar: function(){ return getWorkCalendarV224(payload); },
    inbox: function(){ return getMyWorkInboxV23(payload); },
    relatedTask: function(){ return createRelatedTaskV23(payload); },
    duplicates: function(){ return findTaskDuplicatesV23(payload); },
    saveProjectReportSetting: function(){ return saveProjectReportSettingV23(payload); },
    nasExport: function(){ return exportNasPackV23(payload); },
    ...
    install: function(){ return installPssAiPmoV23(); }
  };
